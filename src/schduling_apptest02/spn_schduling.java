...
class spn_schduling {
	...
	spn_schduling(make_processes tum,int p,int e){
		new_ps = tum; //프로세스 생성
	...
	void bt_solt(int t) { //bt작은순 정렬
		for(int i = 0;i<ready.size()-1;i++) {
			for(int j = i+1;j<ready.size();j++) {
				if(new_ps.processes_list.get(ready.get(i)).time[1] > new_ps.processes_list.get(ready.get(j)).time[1])
					Collections.swap(ready,i,j);
			}
		}
		for(int i = 0;i<ready.size();i++) {
			System.out.println("t: "+t +", p"+(ready.get(i)+1)+" bt : " + new_ps.processes_list.get(ready.get(i)).time[1]);
		}
	}
